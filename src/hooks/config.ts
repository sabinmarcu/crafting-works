...
export const groupCollapsed = (...args: any[]) =>
  _shouldLog && console.groupCollapsed(...args);
// eslint-disable-next-line no-console
...
const makeMatcher = (pf?: string) =>
  new RegExp(["^", _prefix, ...(pf ? [":", pf] : [])].join(""));
export const makeKey = (key: string, pf?: string) =>
  [_prefix, [pf, key].join("")].join(":");
export const isKey = (key: string, pf?: string) => key.match(makeMatcher(pf));
export const stripKey = (key: string, pf?: string) =>
  key.replace(makeMatcher(pf), "");
